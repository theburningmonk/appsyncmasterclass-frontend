    ...
    <div class="flex container h-screen w-full">
      ...
      <div class="w-1/2 h-full overflow-y-scroll">
        ...
        <div class="flex flex-row justify-evenly mt-2">
          <button class="w-1/2 text-dark font-bold border-b-2 border-blue px-10 py-4 hover:bg-lightblue">Followers</button>
          <button @click="goToFollowing()" class="w-1/2 text-dark font-bold border-b-2 px-10 py-4 hover:bg-lightblue">Following</button>
        </div>
...
export default {
  ...
  async created() {
    ...
    await this.getFollowers({ userId: this.profile.id, limit:10 });
  }
}
</script>